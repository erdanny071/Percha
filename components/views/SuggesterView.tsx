...
export function SuggesterView({ occasion='informal', onBack }: Props) {
 ...
 const combos = useMemo(() => generateAllOutfits(garments, occasion, log, true, true), [garments, occasion, log]);
 ...
 if (selected) return <OutfitEditor combo={selected} garments={garments} onBack={()=>setSelected(null)} onSave={createOutfit} onWear={async ids=>{await createOutfit(ids);await applyOutfit(ids);onBack()}}/>;
 return <section className="percha-panel">
  <header className="app-header"><button className="back-button" onClick={onBack}>←</button><div><p className="eyebrow">Sugerencia inteligente</p><h1>¿Qué me pongo?</h1><p>{occasion}</p></div></header>
