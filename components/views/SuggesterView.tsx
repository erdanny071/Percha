...
export function SuggesterView({ occasion='informal', onBack }: Props) {
 ...
 if (selected) return <OutfitEditor combo={selected} garments={garments} onBack={()=>setSelected(null)} onWear={async ids=>{await addOutfit({id:crypto.randomUUID(),name:`Outfit ${new Date().toLocaleDateString('es-ES')}`,garmentIds:ids,createdAt:Date.now(),updatedAt:Date.now()});await applyOutfit(ids);onBack()}}/>;
